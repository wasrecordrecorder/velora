...
public final class ScriptTransactionImpl implements ScriptTransaction {
    ...
    private boolean validateSettings() {
        ...
        if (settingSchema == null) return true;
        for (Map.Entry<String, SettingValue> entry : settingUpdates.entrySet()) {
            String settingId = entry.getKey();
            SettingValue value = entry.getValue();
            Optional<SettingDescriptor> desc = settingSchema.find(settingId);
            if (desc.isEmpty()) {
                return false;
            }
            SettingDescriptor d = desc.get();
            if (!isSettingTypeCompatible(value.type(), d.type())) {
                return false;
            }
            for (SettingDescriptor.Constraint c : d.constraints()) {
                if (c.kind() == SettingDescriptor.Constraint.Kind.RANGE) {
                    if (!checkRange(value, c)) {
                        return false;
                    }
                }
            }
    ...
    private boolean isSettingTypeCompatible(VeloraType valueType, VeloraType expectedType) {
        if (valueType == expectedType) return true;
        if (expectedType == VeloraTypes.DOUBLE && valueType == VeloraTypes.INT) return true;
        if (expectedType == VeloraTypes.DOUBLE && valueType == VeloraTypes.LONG) return true;
        if (expectedType == VeloraTypes.LONG && valueType == VeloraTypes.INT) return true;
        return false;
    }

    private boolean checkRange(SettingValue value, SettingDescriptor.Constraint c) {
        if (!(value.value() instanceof Number n)) return true;
        double v = n.doubleValue();
        if (c.min() instanceof Number min) {
            if (v < min.doubleValue()) return false;
        }
        if (c.max() instanceof Number max) {
            if (v > max.doubleValue()) return false;
        }
        return true;
    }
